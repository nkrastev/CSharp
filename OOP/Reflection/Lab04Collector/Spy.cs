...
namespace Stealer
{
    public class Spy
    {
        ...
        public string CollectGettersAndSetters(string className)
        {
            ...
            MethodInfo[] allGettersSetters = classForSpy.GetMethods(
                BindingFlags.NonPublic | 
                BindingFlags.Instance | 
                BindingFlags.Public | 
                BindingFlags.Static);
            
            foreach (MethodInfo item in allGettersSetters.Where(m=>m.Name.StartsWith("get")))
            {
                sb.AppendLine($"{item.Name} will return {item.ReturnType}");
            }

            foreach (MethodInfo item in allGettersSetters.Where(m => m.Name.StartsWith("set")))
            {
